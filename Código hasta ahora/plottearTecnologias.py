...
def obtener_tecnologias():
    ...
    with open("tecnologias_a_buscar.txt") as file_object:
    ...
    for line in range (lines['Tecnologias'].shape[0]):
        for j in range (result_f["Palabras"].shape[0]):
            if (result_f.loc[j,'Palabras']) == lines.loc[line,'Tecnologias']:
                if contador2 == 0:
                    lista_frec = [result_f.loc[j,'Repeticiones']]
                    lista_tecno = [result_f.loc[j,'Palabras']]
                    contador2 += 1
                else:
    ...
    df_t = pd.DataFrame()
    df_t['Tecnologia'] = None
    df_t['Frecuencia'] = None
