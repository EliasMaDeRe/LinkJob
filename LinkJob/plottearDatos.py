...
def plottear_modalidad_EG(CMT):
    ...
    plt.bar(CMT['Modalidad'], CMT['Empresa Grande'], color = "darkorange", width= .35)
...
def plottear_modalidad_EM(CMT):
    ...
    plt.bar(CMT['Modalidad'], CMT['Empresa Mediana'], color = "darkorange", width=.35)
...
def plottear_modalidad_EP(CMT):
    ...
    plt.bar(CMT['Modalidad'], CMT['Empresa Pequena'], color = "darkorange", width=.35)
...
def plottear_TE_vir(CTM):
    ...
    for i in range (CTM.shape[0]):
        plt.annotate(CTM.loc[i,'Virtual'], xy=(CTM.loc[i,'Tamaño de empresa'], (CTM.loc[i,'Virtual'])+0.03), ha = 'center')
        
    plt.title('Cantidad de ofertas por tamaño de empresa en modalidad virtual')
    plt.ylabel('Cantidad de ofertas')
    plt.xlabel('Tamaño de empresa')
    plt.annotate('Total de ofertas: ' + CTM['Total de ofertas Presencial'].loc[0], xy = ('Pequeña', CTM['Virtual'].loc[0]-0.25), ha = 'center')
    plt.savefig('testplot.png',dpi=300,bbox_inches='tight')
    plt.close()
...
def plottear_TE_pres(CTM):
    ...
    for i in range (CTM.shape[0]):
        plt.annotate(CTM.loc[i,'Presencial'], xy=(CTM.loc[i,'Tamaño de empresa'], (CTM.loc[i,'Presencial']+5)), ha = 'center')
        
    plt.title('Cantidad de ofertas por tamaño de empresa en modalidad presencial')
    plt.ylabel('Cantidad de ofertas')
    plt.xlabel('Tamaño de empresa')
    plt.annotate('Total de ofertas: ' + CTM['Total de ofertas Presencial'].loc[0], xy = ('Pequeña', CTM['Presencial'].loc[0] -20 ), ha = 'center')
    plt.savefig('testplot.png',dpi=300,bbox_inches='tight')
    plt.close()
...
def plottear_salario_eg (df):
    ...
    dfsalario_grande= dfsalario_grande_ambos.drop(['Pequeña'])#.drop(['Mediana']) decomentar si hay ofertas de empresas medianas
    tplot = dfsalario_grande.plot(xlabel ="Tamaño",ylabel="Salario en pesos",color = "darkorange", kind='bar', title = "Salario promedio por empresas grandes en México",width =.05,legend=False)
    tplot.annotate('Total de ofertas: '+str(df4['Cantidad'].sum()), xy=(.2,df4['Promedio'].max()*.8), ha='center', va='bottom',fontsize=8)
    for p in tplot.patches:
        tplot.annotate(str(p.get_height()),xy=((p.get_x()),p.get_height()),ha='left',va='bottom')
...
def plottear_salario_em (df):
    ...
    dfsalario_grande= dfsalario_grande_ambos.drop(['Grande']).drop(['Mediana'])
    tplot = dfsalario_grande.plot(xlabel ="Tamaño",ylabel="Salario en pesos",color = "darkorange", kind='bar', title = "Salario promedio por empresas medianas en México", width =.05,legend=False)
    tplot.annotate('Total de ofertas: '+str(df4['Cantidad'].sum()), xy=(.2,df4['Promedio'].max()*.8), ha='center', va='bottom')
    for p in tplot.patches:
        tplot.annotate(str(p.get_height()),xy=((p.get_x()),p.get_height()),ha='left',va='bottom')
...
def plottear_salario_ep (df):
    ...
    dfsalario_grande= dfsalario_grande_ambos.drop(['Grande'])#.drop(['Mediana']) descomentar si hay ofertas de empresas medianas con salario
    tplot = dfsalario_grande.plot(xlabel ="Tamaño",ylabel="Salario en pesos",color = "darkorange", kind='bar', title = "Salario promedio por empresas pequeñas en México", width =.05,legend=False)
    tplot.annotate('Total de ofertas: '+str(df4['Cantidad'].sum()), xy=(.15,dfsalario_grande.max()*.8), ha='center', va='bottom')
    for p in tplot.patches:
        tplot.annotate(str(p.get_height()),xy=((p.get_x()),p.get_height()),ha='left',va='bottom')
...
def plottear_salario_presencial(df):
    ...
    tplot = dfsalario_grande.plot(xlabel ="Modalidad",ylabel="Salario en pesos",color = "darkorange", kind='bar', title = "Salario promedio por modalidad presencial en México", width =.05,legend=False)
    tplot.annotate('Total de ofertas: '+str(df4['Cantidad'].sum()), xy=(.15,dfsalario_grande.max()*.8), ha='center', va='bottom')
    for p in tplot.patches:
        tplot.annotate(str(p.get_height()),xy=((p.get_x()),p.get_height()),ha='left',va='bottom')
...
def plottear_salario_virtual(df):
    ...
    tplot = dfsalario_grande.plot(xlabel ="Modalidad",ylabel="Salario en pesos",color = "darkorange", kind='bar', title = "Salario promedio por modalidad virtual en México", width =.05,legend=False)
    tplot.annotate('Total de ofertas: '+str(df4['Cantidad'].sum()), xy=(.15,dfsalario_grande.max()*.8), ha='center', va='bottom')
    for p in tplot.patches:
        tplot.annotate(str(p.get_height()),xy=((p.get_x()),p.get_height()),ha='left',va='bottom')
