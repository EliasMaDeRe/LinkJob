...
def mostrar_imagen_faltainf():
    imagen = Image.open('noexisteinfo.png')
    imagen = imagen.resize((largo, alto), Image.ANTIALIAS)
...
def imagen_categorias(valor):
    ...
    # Filtros generales con específicos
    ...
    elif filtro_general == 'Modalidad' and filtro_esp == 'Presencial':
        ML = pd.read_csv("ML.csv")
        pld.plottear_modalidad(ML)
        mostrar_imagen()

    elif filtro_general == 'Modalidad' and filtro_esp == 'Virtual':
        ML = pd.read_csv("ML.csv")
        pld.plottear_modalidad(ML)
        mostrar_imagen()

    elif filtro_general == 'Locación' and filtro_esp == 'Presencial':
        mostrar_imagen_faltainf()
        ...
        CTM = pd.read_csv("CTM.csv")
        pld.plottear_TE_vir(CTM)
        mostrar_imagen()

    elif filtro_general == 'Tamaño de empresa' and filtro_esp == 'Empresa Grande':
        TL = pd.read_csv("TL.csv")
        pld.plottear_tamano_empresa(TL)
        mostrar_imagen()

    elif filtro_general == 'Tamaño de empresa' and filtro_esp == 'Empresa Mediana':
        TL = pd.read_csv("TL.csv")
        pld.plottear_tamano_empresa(TL)
        mostrar_imagen()

    elif filtro_general == 'Tamaño de empresa' and filtro_esp == 'Empresa Pequeña':
        TL = pd.read_csv("TL.csv")
        pld.plottear_tamano_empresa(TL)
        mostrar_imagen()
...
logo = logo.resize((240, 75), Image.ANTIALIAS)

log = ImageTk.PhotoImage(logo)
mostrarlogo = Label(ventana, image=log)
mostrarlogo.config(bg=fondo)
mostrarlogo.place(relx=0.6, rely=0, relwidth=1, anchor='ne')
...
# Las opciones que tendrá el dropdown específico
opcionesesp = ["Ninguno", "Presencial", "Virtual",
...
dropdowngen = OptionMenu(
    ventana, clickgen, *opcionesgen, command=imagen_categorias)
dropdowngen.config(bg=fondo)
dropdowngen.place(relx=0.49, rely=0.95, relwidth=0.3, anchor='ne')

dropdownesp = OptionMenu(
    ventana, clickesp, *opcionesesp, command=imagen_categorias)
dropdownesp.config(bg=fondo)
dropdownesp.place(relx=0.79, rely=0.95, relwidth=0.3, anchor='ne')

# Empezar con una imagen por defecto
imagen = Image.open('seleccionelosfiltros.png')
imagen = imagen.resize((largo, alto), Image.ANTIALIAS)
